...
var RussianMap = function(options, regions) {
    ...
    var onMouseOut = function(event) {
        ...
        if (region.paths !== undefined) {
            for (var p in region.paths) {
                region.paths[p].attr(get_region_attr(region.ident, $("#MainContent_DiseaseDropDownList").val()) || defaultAttr);
            }
        }
        if (region.polygons !== undefined) {
            for (var p in region.polygons) {
                region.polygons[p].attr(get_region_attr(region.ident, $("#MainContent_DiseaseDropDownList").val()) || defaultAttr);
            }
        }
    ...
    var renderRegion = function (region) {
        ...
        if (region.paths !== undefined) {
            for (var p in region.paths) {
                var path = R.path(region.paths[p]).attr(get_region_attr(region.ident, $("#MainContent_DiseaseDropDownList").val()) || defaultAttr);
                path.region = region;
        ...
        if (region.polygons !== undefined) {
            for (var p in region.polygons) {
                var polygon = R.path('M' + region.polygons[p]).attr(get_region_attr(region.ident, $("#MainContent_DiseaseDropDownList").val()) || defaultAttr);
